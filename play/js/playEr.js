...
// Защита: если кликнули по кнопке, слайдеру или плейлисту — игнорируем паузу
// if (event && (event.target.closest('button') || event.target.closest('input') || event.target.closest('.track-item'))) {
//   return;
// }
// 1. Проверяем наличие объекта песни в ОБЩЕМ хранилище
// if (!window.playerData.song) return;

// 2. Управляем воспроизведением через ОБЩЕЕ хранилище
// if (window.playerData.song.isPlaying()) {
//   window.playerData.song.pause();

// Обновляем текст кнопки
// const playbtnEr = document.getElementById('play-btnEr');
// if (playbtnEr) playbtnEr.innerText = "PLAY";

// Можно также обновить статус плеера для визуализатора // Пауза
//   window.playerData.playerState = 2; 
// } else {
//   window.playerData.song.play();

// Обновляем текст кнопки
// const playbtnEr = document.getElementById('play-btnEr');
// if (playbtnEr) playbtnEr.innerText = "PAUSE";

// Обновляем статус плеера // Играет
