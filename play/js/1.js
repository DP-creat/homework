...
function handleMainClick() {
  ...
  const labels = ["ARC-9", "HUD ON", "GESTURE"];
  btnText.innerText = labels[window.playerData.playerState];
}



// отслеживать движение пальца или мышки.
...
document.addEventListener('mousedown', (e) => {
  if (window.playerData.playerState === 2) { // Работает только в state-2
...
document.addEventListener('mouseup', (e) => {
  if (window.playerData.playerState !== 2) return;

  let diffX = e.clientX - touchStartX;
  let diffY = e.clientY - touchStartY;
  let threshold = 50; // Чувствительность в пикселях

  // ГОРРИЗОНТАЛЬ (Треки)
  if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > threshold) {
    if (diffX > 0) prevTrack(); // Свайп вправо
    else nextTrack();           // Свайп влево
  }

  // ВЕРТИКАЛЬ (Громкость)
  if (Math.abs(diffY) > Math.abs(diffX) && Math.abs(diffY) > threshold) {
    let currentVol = parseFloat(document.getElementById('volume-slider').value);
    if (diffY < 0) { // Свайп вверх
      updateVolume(Math.min(currentVol + 0.1, 1));
    } else { // Свайп вниз
      updateVolume(Math.max(currentVol - 0.1, 0));
    }
    // Обновляем положение ползунка визуально
    document.getElementById('volume-slider').value = window.playerData.song.getVolume();
  }
});

// Аналог для мобилок (Touch)
...
window.togglePanel = (name) => {
    ...
    // Если открыли/закрыли панель плеера — вешаем флаг на body
    if (name === 'playEr') {
        document.body.classList.toggle('panel-open');
    }

    // Твой старый код скролла...
    document.body.style.overflow = document.querySelector('.panel.active') ? 'hidden' : '';
};