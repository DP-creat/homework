...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =============BIO ANALYSE++++++++++++++++
    window.updateBioCalc = () => {
        const stress = parseInt(document.getElementById('stress-range').value);
        const sugar = parseInt(document.getElementById('sugar-range').value);
        const sleep = parseInt(document.getElementById('sleep-range').value);

        // Формула "просадки"
        const depletion = Math.round((stress * 0.5) + (sugar * 0.2) + (sleep * 0.3));

        const resVal = document.getElementById('burnout-percent');
        const advice = document.getElementById('bio-advice');

        resVal.innerText = `${depletion}%`;

        // Цвет текста в зависимости от урона
        resVal.style.color = depletion > 60 ? 'var(--red)' : 'var(--cyan)';

        // Логика советов
        let text = "STATUS: STABLE. No intervention needed.";
        if (depletion > 20) text = "ADVICE: Magnesium + Omega-3 to stabilize nerves.";
        if (depletion > 45) text = "CRITICAL: Tyrosine for dopamine support. Stop Sugar.";
        if (depletion > 70) text = "WARNING: 5-HTP + Deep Sleep cycle required immediately.";
