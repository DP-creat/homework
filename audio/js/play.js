...
function handleMainClick() {
  ...
  // Логика текста и значков внутри круга
  if (playerState === 1) {
    // СОСТОЯНИЕ 1: ПОЛНЫЙ HUD (Плейлист, Кнопки, Инфо)
    btnText.innerHTML = "<span>▶</span>"; // Значок плей в центре
    if (playlist.length > 0 && (!song || !song.isPlaying())) togglePlay();
  }
  else if (playerState === 2) {
    // СОСТОЯНИЕ 2: ЧИСТЫЙ РЕЖИМ (Только дорожка и название)
...
async function syncSystemAudio() {
  try {
    ...
    document.getElementById('sync-btn').style.background = "var(--electric-sky)";
    document.getElementById('sync-btn').style.color = "var(--bg-cos)";
...
class Bolt {
  ...
  draw() {
    ...
    // Основная нить (Тонкая)
    stroke(this.hue, 70, 100, this.life);
    strokeWeight(1);
    noFill();
    beginShape();
