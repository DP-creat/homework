...
// 5. КИСЛОТНЫЕ ВСПЫШКИ (Нейроны)
  ...
  for (let a = 0; a < TWO_PI; a += 0.05) {
    ...
    // Масштабируем и выводим точку
    vertex(hx * (ringR / 15), hy * (ringR / 15));
  }
  endShape(CLOSE); // Замыкаем контур, чтобы не было дыр

  // --- МОЛНИИ (Бьют теперь из контура нового сердца) ---
  if (bass > 200 && frameCount % 2 === 0) {
    let randomAngle = random(TWO_PI);
    // Считаем координаты точки на сердце для удара молнии
