...
function playTrack(index) {
  ...
  if (window.playerData.song) {
    // Сначала обнуляем обработчик, чтобы stop() не вызвал автопереход!
    window.playerData.song.onended(() => {}); 
    window.playerData.song.stop();
    window.playerData.song.disconnect(); // Отключаем от аудио-выхода
    window.playerData.song = null; // Полностью стираем из памяти
  }
  ...
  window.playerData.song = loadSound(window.playerData.playlist[index].url, () => {
    if (window.playerData.song.isLoaded()) {
      // ПРОВЕРКА ГРОМКОСТИ
      // Ищем слайдер. Если его нет на текущей панели — ставим 0.5 по умолчанию
      const volSlider = document.getElementById('volume-slider');
      let volume = volSlider ? parseFloat(volSlider.value) : 0.5;
      window.playerData.song.setVolume(volume);
      window.playerData.song.play();

      // СВЯЗКА: Когда песня кончится, вызываем функцию переключения
      window.playerData.song.onended(() => {
        const s = window.playerData.song;
        // Проверяем: если время почти равно длительности, значит песня реально кончилась
        // Если время маленькое (после jump), переключение не сработает
        if (s.currentTime() >= s.duration() - 0.5 || s.currentTime() === 0) {
          window.nextTrack();
        }
      });
...
function seek(e) {
  if (!window.playerData.song || !window.playerData.song.isLoaded()) return;
  
  // Чтобы клик не вызывал срабатывание других элементов под прогресс-баром
  e.stopPropagation();
...
function handleMainClick() {
  ...
  // УДАЛЕНИЕ НАДПИСИ: убираем "click" при самом первом нажатии
  const clickLabel = document.getElementById('click-label');
  if (clickLabel) {
    clickLabel.remove();
  }

  // 1. УМНАЯ СМЕНА СОСТОЯНИЙ (Исключаем 0 после первого клика)
  if (window.playerData.playerState === 0) {
    // Если это самый первый клик, сразу включаем ИГРУ (state-1)
    window.playerData.playerState = 1;
  } else {
    // В последующие разы строго чередуем: если 1 -> станет 2, если 2 -> станет 1
    window.playerData.playerState = window.playerData.playerState === 1 ? 2 : 1;
  }

  // 2. Синхронизация визуальных состояний слоя
  if (layer) {
    // Сбрасываем старые классы анимации (удалять state-1 и state-2 нужно, чтобы они сменяли друг друга)
    layer.classList.remove('state-1', 'state-2');

    // Добавляем актуальный класс (state-1 или state-2)
    layer.classList.add(`state-${window.playerData.playerState}`);
  }

  // 3. Обновление текста центральной кнопки
  if (btnErText) {
    // Массив подстраиваем под индексы: 0-пусто, 1-игра, 2-пауза
    const states = ["", "", ""]; 
