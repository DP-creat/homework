...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateScannerDisplay = () => {
        ...
        if (scanner && !scanner.classList.contains('active') && !scanner.classList.contains('running')) {
            ...
            if (labEl) labEl.innerText = "REMAINING RESOURCE";
        }
    };
        ...
        if (type === 'log') {
            ...
            if (logWindow) {
                ...
                if (logWindow.classList.contains('active')) {
                    logWindow.scrollTo({ top: 0, behavior: 'smooth' });
                }
            }
        } else {
    ...
    window.toggleChrono = () => {
        ...
        // ПЕРВЫЙ КЛИК: Активация (Загорается кольцо)
        if (!scanner.classList.contains('active')) {
            scanner.classList.add('active');
            ring.style.boxShadow = "0 0 20px var(--cyan)";
            label.innerText = "CHRONO READY";
            return;
        }

        // ВТОРОЙ КЛИК: Запуск секундомера (Кольцо начинает крутиться)
        if (!scanner.classList.contains('running')) {
            scanner.classList.add('running');
            ring.style.animationPlayState = "running"; // Запуск анимации вращения
            label.innerText = "ELAPSED TIME";

            chronoInterval = setInterval(() => {
                chronoSeconds++;
                const mins = Math.floor(chronoSeconds / 60).toString().padStart(2, '0');
                const secs = (chronoSeconds % 60).toString().padStart(2, '0');
                value.innerText = `${mins}:${secs}`;
            }, 1000);
        } else {
            // ТРЕТИЙ КЛИК: Остановка и сброс
