document.addEventListener('DOMContentLoaded', () => {
    ...
    window.toggleCockpit = () => {
        const cockpit = document.querySelector('.hud-panel.cockpit');
        if (cockpit) {
            cockpit.classList.toggle('active');
        }
    };
    window.togglePanel = (type) => {
    const target = document.querySelector(`.hud-panel.${type}`);
    if (!target) return;

    // Закрываем только КИНЕТИК и КЛИКЕР (не трогаем кокпит)
    document.querySelectorAll('.hud-panel:not(.cockpit)').forEach(p => {
